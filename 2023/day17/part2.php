...
define("MIN_SAME_STEPS", 4);
define("MAX_SAME_STEPS", 10);
...
function is_valid_stop(array $prev_steps): bool {
	if (
		($last = last($prev_steps, MIN_SAME_STEPS))
...
function invalid_steps(array $prev_steps): int {
	...
	if (!is_valid_stop($prev_steps)) {
		return array_sum(DIRECTIONS) - last($prev_steps);
	}
